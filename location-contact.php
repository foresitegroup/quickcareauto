<?php
$PageTitle = "Location/Contact";
$HeaderImg = "location-contact.jpg";
include "header.php";
?>

<div style="float: right; text-align: right; margin-left: 50px;">
  <iframe width="460" height="460" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.com/maps?q=185+S+Main+St.+Thiensville,+WI+53092&amp;oe=utf-8&amp;ie=UTF8&amp;hl=en&amp;hq=&amp;hnear=185+S+Main+St,+Thiensville,+Ozaukee,+Wisconsin+53092&amp;source=embed&amp;ll=43.22832,-87.984867&amp;spn=0.025016,0.034246&amp;z=14&amp;iwloc=I&amp;output=embed"></iframe>
  <br>
  <small><a href="http://maps.google.com/maps?q=185+S+Main+St.+Thiensville,+WI+53092&amp;oe=utf-8&amp;ie=UTF8&amp;hl=en&amp;hq=&amp;hnear=185+S+Main+St,+Thiensville,+Ozaukee,+Wisconsin+53092&amp;source=embed&amp;ll=43.22832,-87.984867&amp;spn=0.025016,0.034246&amp;z=14&amp;iwloc=A">View Larger Map</a></small>
</div>

Quick Care Auto<br>
185 S. Main Street<br>
Thiensville, WI 53092<br>
<br>
Phone: 262-242-0747<br>
<!--Detailing Scheduling: 262-242-0747<br>-->
<?php email("customerservice@quickcareauto.com"); ?><br>
<br>
Monday - Friday 8:00am to 5:00 pm<br>
Saturday closed<br>
<br>

<h2>Directions</h2>

<strong>From Milwaukee</strong><br>
<ul>
  <li>43 North to Mequon Rd. exit.</li>
  <li>Take Mequon Rd. west 3.1 miles.</li>
  <li>Turn north on Cedarburg Rd.</li>
  <li>Continue onto Main St. 0.5 miles.</li>
  <li>Quick Care Auto is the second building on your left north of Spring St.</li>
</ul>

<br>

<strong>From Cedarburg</strong><br>
<ul>
  <li>Travel Washington Ave. and Cedarburg Rd. south 3.7 miles.</li>
  <li>Continue on Main St. south 1.1 miles.</li>
  <li>Quick Care Auto is on the west side of street two blocks south of Buntrock Ave.</li>
</ul>

<?php include "footer.php"; ?>